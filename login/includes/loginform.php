...
class LoginForm extends DbConn
{
    public function checkLogin($myusername, $mypassword)
    {
        ...
        $stmt = $db->conn->prepare("SELECT * FROM ".$tbl_members." WHERE username = :myusername OR email = :myusername");
        ...
        if ($curr_attempts >= $max_attempts && $timeDiff < $login_timeout) {
            ...
            $success = "<div class=\"alert alert-danger alert-dismissable\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button>Maximo numero de intentos excedidos...por favor esperar ".$timeout_minutes." minutos antes de intentar iniciar sesion de nuevo.</div>";
            ...
            if (password_verify($mypassword, $result['password']) && $result['verified'] == '1') {
                ...
                $success = "<div class=\"alert alert-danger alert-dismissable\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button>Usuario o Contraseña equivocada.</div>";
